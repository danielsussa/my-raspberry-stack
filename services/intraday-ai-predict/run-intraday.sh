...
PROMPT_FILE="${INTRADAY_AI_PREDICT_PROMPT_FILE:-/data/intraday-ai-predict/AI_README.md}"
...
CODEX_ARGS="${INTRADAY_AI_PREDICT_CODEX_ARGS:---dangerously-bypass-approvals-and-sandbox}"
...
if echo "$CODEX_ARGS" | grep -Fq -- "--dangerously-bypass-approvals-and-sandbox"; then
  MODE_ARGS=""
fi
...
if echo "$output" | grep -Fq "$STOP_TOKEN"; then
  {
    echo "## $(date -Iseconds)"
    echo "$output"
    echo
  } >> "$OUT_FILE"
  echo "Stop token received ($STOP_TOKEN)."
  exit 99
fi
